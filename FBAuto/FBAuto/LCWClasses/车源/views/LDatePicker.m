...
//        [datePicker addTarget:self action:@selector(updatetime:) forControlEvents:UIControlEventValueChanged];
        ...
        datePicker = [[SRMonthPicker alloc]initWithFrame:CGRectMake(0, 44, self.width, 216)];
        datePicker.maximumYear = @3000;
...
- (void)monthPickerWillChangeDate:(SRMonthPicker *)monthPicker
{
    ...
    NSLog(@"monthPickerWillChangeDate-->%@",[self selectTime:monthPicker.date]);
...
- (void)monthPickerDidChangeDate:(SRMonthPicker *)monthPicker
{
    ...
    dispatch_async(delayQueue, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (dateBlock) {
                dateBlock([self selectTime:monthPicker.date]);
            }
        });
    });
